...
# Toggle one "true" the rest "false" for wanted case
_single = false # Single Flettner rotor case
_validation_free_slip = false # Validation case (free-slip BC at bottom) 
_validation_symmetry = true # Validation case (symmetry BC at bottom) 
_validation = false # Validation case (full domain) 
if _validation_symmetry 
...
pstep = 0.25
clims_u = (-1,3) # Velocity plot limits
ustep = 0.25
clims_c = (-5,5) # Curl plot limits
clength = 20
vectors = true # false: do not plot force vectors, true: plot force vectors
fscale = 15.0 # Force vector scale
...
if _single
    ...
    σ_init = 10 # Timestep to start calculatiing standart deviation
    datadir = string(@__DIR__) * "../../sims/data/rotor_BiotSimulation_Single/"
    psolverdir = string(@__DIR__) * "../../"
    pdf_file = "../sims/tex/img/rotor_BiotSimulation_Single.pdf"
elseif _validation_free_slip
    L = (12,5,6) 
    Re = 500
    time_max = 20
    stats_init = 10 
    stats_trim = 10
    σ_init = 10
    datadir = string(@__DIR__) * "../../sims/data/rotor_BiotSimulation_Validation_Free_Slip/"
    psolverdir = string(@__DIR__) * "../../"
    pdf_file = "../sims/tex/img/rotor_Validation_Free_Slip.pdf"
elseif _validation_symmetry
    L = (12,5,6) 
    Re = 500
    time_max = 20 
    stats_init = 10
    stats_trim = 10
    σ_init = 10
    datadir = string(@__DIR__) * "../../sims/data/rotor_BiotSimulation_Validation_Symmetry/"
    psolverdir = string(@__DIR__) * "../../"
    pdf_file = "../sims/tex/img/rotor_Validation_Symmetry.pdf"
elseif _validation
    L = (12,5,12)
    Re = 500
    time_max = 20
    ...
    stats_trim = 10
    σ_init = 10
    datadir = string(@__DIR__) * "../../sims/data/rotor_BiotSimulation_Validation_Full/"
    psolverdir = string(@__DIR__) * "../../"
    pdf_file = "../sims/tex/img/rotor_Validation_Full.pdf"
end 
...
function main(U=U)
    ...
    for D in Ds
        for λ₁ in λs₁
            x_vals = T.(1.9:1/D:8)
            println("Running case: D = $(D), λ = $(λ₁), θ = $(θ)")
            if run && _single
                sim, meanflow, p_force₁, v_force₁ = run_sim_rotor_NonBiotFaces_Single(D, λ₁, θ, U₊, backend;dir=datadir, L, u_probe_loc, u_probe_component, Re, T, restart=false, cont=_continue, log_tag=2)
            elseif run && _validation_free_slip
                sim, meanflow, p_force₁, v_force₁ = run_sim_rotor_Validation_Free_Slip(D, λ₁, θ, U₊, backend;dir=datadir, L, u_probe_loc, u_probe_component, Re, T, restart=false, cont=_continue, log_tag=2)
            elseif run && _validation_symmetry
                sim, meanflow, p_force₁, v_force₁ = run_sim_rotor_Validation_Free_Slip(D, λ₁, θ, U₊, backend;dir=datadir, L, u_probe_loc, u_probe_component, Re, T, restart=false, cont=_continue, log_tag=2)
            elseif run && _validation
                sim, meanflow, p_force₁, v_force₁ = run_sim_rotor_Validation(D, λ₁, θ, U₊, backend;dir=datadir, L, u_probe_loc, u_probe_component, Re, T, restart=false, cont=_continue, log_tag=2)
            end

            if _single
                tag = "Single"
            elseif _validation_free_slip
                tag = "validation_free_slip"
            elseif _validation_symmetry
                tag = "validation_symmetry"
            elseif _validation
                tag = "validation_full"
            end

            p_force₁, v_force₁, u_probe_x_full, u_probe_y_full, u_probe_z_full, t_full = read_force_and_probe_vals_single("force_rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_Single.jld2"; dir=datadir)
            forces = get_forces_and_coefficients_single(p_force₁, v_force₁, D)
            meanforces = get_mean_forces_and_coefficients_single(forces, t_full, stats_init)
            ...
            if stats && stats_turb
                P, U, UU, τ, t_meanflow = read_meanflow("meanflow_rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_Single.jld2"; dir=datadir, stats, stats_turb)
                @out size(τ)
                Ux, Uy, Uz = U[:,:,:,1], U[:,:,:,2], U[:,:,:,3]
            else stats
                P, U, t_meanflow = read_meanflow("meanflow_rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_Single.jld2"; dir=datadir, stats, stats_turb)
                Ux, Uy, Uz = U[:,:,:,1], U[:,:,:,2], U[:,:,:,3]
            end
            ...
            p, u, Δt = read_flow("flow_rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_Single.jld2";dir=datadir)

            out_file = joinpath(datadir, "UW_planes_$(D)_$(λ₁)_$(θ)_$(tag).jld2")
            jldsave(out_file; 
                U_planes = U_planes,
                W_planes = W_planes,
                z_vals = z_vals,
            ...
            out_file = joinpath(datadir, "curl_$(D)_$(λ₁)_$(θ)_$(tag).jld2")
            ...
            if _plot 
                ...
                for i in 1:size(U_planes, 1)
                    ...
                    ylabel =L"\overline{u}/U_{\infty}", framestyle=:box, grid=true, size(600,600), xlims=(x_vals[1], x_vals[end]),
                    tickfontsize=10, labelfontsize=10, legendfontsize=10, legend = :bottomright, left_margin=Plots.Measures.Length(:mm, 5), lw=2)
                    savefig(plt_Ux,string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_Ux.pdf")

                    Plots.plot!(plt_Uz,x_vals, W_planes[i, :], label = @sprintf("z/D = %.2f", z_vals[i]), xlabel =L"x/D",
                    ylabel =L"\overline{w}/U_{\infty}", framestyle=:box, grid=true, size(600,600), xlims=(x_vals[1], x_vals[end]),
                    tickfontsize=10, labelfontsize=10, legendfontsize=10, legend = :topright, left_margin=Plots.Measures.Length(:mm, 5), lw=2)
                    savefig(plt_Uz,string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_Uz.pdf")
                end
                ...
                savefig(plt_P,string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_P.pdf")
                ...
                savefig(plt_p,string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_P_instantaneous.pdf")
                ...
                for z in z_vals.*D
                    ...
                    Plots.contour!(permutedims(Ux[:,:,Int(z)]), levels=range(clims_u[1],clims_u[2],step=ustep), color=:black, linewidth=1)
                    ...
                    savefig(plt_Ux_flood, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_Ux_flood_at_$(z/D).pdf")
                end
                ...
                for z in z_vals.*D
                    ...
                    ϕ = range(0, 2π; length=100)
                    Plots.plot!(o₁[1] .+ r*cos.(ϕ), o₁[2] .+ r*sin.(ϕ), color=:red, lw=2, label="")
                    savefig(plt_curl_z, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_curl_z_at_$(z/D).pdf")
                end
                ...
                for z in z_vals.*D
                    ...
                    savefig(plt_P_flood, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_P_flood_at_$(z/D).pdf")
                end

                Mval = Printf.@sprintf("%.1f", prod(L .* D) / 1e6)
                xD, yD, zD = u_probe_loc
                main_lab = latexstring("u_{x}(t), D=$(D), λ=$(λ₁), \\theta=$(θ), Loc.=($(u_probe_loc[1])D, $(u_probe_loc[2])D, $(u_probe_loc[3])D)")
                avg_lab  = L"u_{x},\ \mathrm{moving\ avg.}"
                sig_lab  = L"u_{x},\ \mathrm{moving}\ \sigma" 
                main_lab_x_y = latexstring("(u_{x}(t),u_{y}(t)), D=$(D), λ=$(λ₁), \\theta=$(θ), Loc.=($(u_probe_loc[1])D, $(u_probe_loc[2])D, $(u_probe_loc[3])D)")
                avg_lab_x_y = L"\mathrm{moving\ avg.}\ (u_{x},u_{y})"
                sig_lab_x_y = L"\mathrm{moving}\ \sigma\ (u_{x},u_{y})"
                main_lab_force = latexstring("C_{L}(t), D=$(D), λ=$(λ₁), \theta=$(θ), Loc.=($(u_probe_loc[1])D, $(u_probe_loc[2])D, $(u_probe_loc[3])D)")
                ...
                plt_u_probe_x = Plots.plot(
                    ...
                    size = (600, 600),
                    xlims = (0, t_full[end]),
                    tickfontsize = 16, labelfontsize = 16, legendfontsize = 12, legend = :top,
                    left_margin = Plots.Measures.Length(:mm, 5),
                ...
                savefig(plt_u_probe_x, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_mean.pdf")
                ...
                savefig(plt_u_probe_x_box, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_box.pdf")
                ...
                savefig(plt_u_probe_x_stats, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_stats.pdf")
                ...
                savefig(plt_u_probe_x_trim, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_trim.pdf")
                ...
                savefig(plt_u_probe_x_trim_box, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_trim_box.pdf")

                plt_u_probe_x_y = Plots.scatter(
                    u_probe_x_full, u_probe_y_full;
                    label = main_lab_x_y,
                ...
                savefig(plt_u_probe_x_y, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_U_probe_x_y.pdf")
                ...
                Plots.plot!(plt_forces, t_full, forces.Cy₁, label=L"C_{L}", lw=2)

                Plots.plot!(plt_forces, [stats_init, t_full[end]], [meanforces.C̄x₁, meanforces.C̄x₁], color=:orange, ls=:dash, lw=2, label=L"\overline{C}_{D}")
                Plots.plot!(plt_forces, [stats_init, t_full[end]], [meanforces.C̄y₁, meanforces.C̄y₁], color=:blue, ls=:dash, lw=2, label=L"\overline{C}_{L}")
                vline!(plt_forces, [stats_init], color=:black, ls=:solid, lw=2, label=false)
                savefig(plt_forces, string(@__DIR__) * "../../tex/img/rotor_BiotSimulation_$(D)_$(λ₁)_$(θ)_$(tag)_forces.pdf")
            end
